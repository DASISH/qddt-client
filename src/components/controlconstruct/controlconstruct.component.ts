...
@Component({
  ...
  directives: [AutocompleteComponent, QddtTableComponent,
  ControlConstructDetailComponent, MaterializeDirective]
})
export class ControlConstructComponent {
  ...
  searchQuestionItems(key: string) {
    this.service.getQuestionItems(key).subscribe(result => {
