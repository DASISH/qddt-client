...
@Component({
  ...
  template: ` <div style="padding-top: 1rem;">
    <div class="row">
      <div class="col left" *ngFor="let domain of domainTypeDescription">
        <label>
          <input
            name="DOMAIN-TYPE-GROUP"
            type="radio"
            (click)="onSelectDomainType(domain.id)"
            [checked]="domainType === domain.id"
          />
...
export class ResponsePreSelector implements OnInit {
  ...
  public readonly domainTypeDescription: {
    id: DomainKind;
    label: string;
  ...
  constructor(
    ...
    if (userService.getRoles().find((p) => p === "ROLE_ADMIN")) {
      this.domainTypeDescription = DOMAIN_TYPE_DESCRIPTION.filter(
        (e) => e.id !== DomainKind.NONE && e.id !== DomainKind.MISSING
      );
    } else {
      this.domainTypeDescription = DOMAIN_TYPE_DESCRIPTION.filter(
        (e) => e.id > DomainKind.NONE && e.id < DomainKind.MISSING
  ...
  private getPageSearch(): IPageSearch {
    return (this.properties.get(this.path) ||
      new PageSearch({ kind: ElementKind.RESPONSEDOMAIN })) as IPageSearch;
  }
}
